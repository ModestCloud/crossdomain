...
module Sinatra
  module CrossOrigin
    module Helpers
      ...
      def cross_origin(hash=nil)
        ...
        if settings.allow_origin.is_a? Array
          origin = settings.allow_origin.include?(request_origin) ? request_origin : settings.allow_origin.join('|')
        else
          origin = settings.allow_origin == :any ? request_origin : settings.allow_origin
        end
    ...
    def self.registered(app)
      ...
      app.set :allow_methods, [:post, :get, :options, :put]
